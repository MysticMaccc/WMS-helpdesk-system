<nav class="side-nav hidden w-[80px] overflow-x-hidden pb-16 pr-5 md:block xl:w-[230px]">
    ...
    <ul>
        ...
        <x-sidebar-link-item icon="edit3" label=" Request List" :status="in_array($activeRoute, ['request.index'])"
            route="request.index" />
        ...
        <x-nested-sidebar-item label="Maintenance"
            ...
            <x-sidebar-link-item icon="sliders" label="Position Maintenance" route="position.index"
                :status="in_array($activeRoute, ['position.index'])" />
            <x-sidebar-link-item icon="sliders" label="User Roles" route="user-role.index"
                :status="in_array($activeRoute, ['user-role.index'])" />

            <x-sidebar-link-item icon="sliders" label="Department" route="department.index"
