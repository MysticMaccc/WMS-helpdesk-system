<x-main-layout :title="$title">
    ...
    <div class="col-span-1 md:col-start-3 md:col-span-5 lg:col-start-3 lg:col-span-6">
        <x-form-section submit="{{ $hash !== null ? 'update' : 'store' }}">
            ...
            <x-slot:title>Category Form</x-slot:title>
            <x-slot:description>Here you can create or edit categories.</x-slot:description>

            <x-slot:form>
                <div class="w-full">
                    <x-input label="Enter category" wireModel="category" wire:model="category" />
                </div>
            </x-slot:form>
            ...
            <x-anchor title="Go back" href="{{ route('category.index') }}" wire:navigate />
        </x-form-section>
    </div>
