<x-main-layout :title="$title">
    ...
    <div class=" col-span-12 text-end">
        <x-list-button title="Generate Form" href="{{ route('request.generate-form', $hash) }}" />
    </div>
    @endif
    <div class=" col-span-1 md:col-start-1 md:col-span-5 lg:col-start-1 lg:col-span-6">
        <x-form-section submit="{{ $hash !== null ? 'update' : 'store' }}" enctype="multipart/form-data">
            ...
            <x-slot:form>
                <div class="w-full">
                    <x-select-input wireModel="requestType" label="Select Request Type" :data="$requestTypeData" wire:model.live="requestType" disabledProp="{{ $disableRequestType }}" />
                </div>
                    ...
                    <x-static-table>
                        ...
                        <x-slot:tbody>
                            ...
                            <x-static-tr>
                                <td class="px-6 py-4 w-40">
                                    <input type="text" class="py-1 w-40" wire:model="details.{{ $index }}" {{ $hash !== null ? 'disabled' : '' }}>
                                </td>
                                <td class="px-6 py-4 w-20">
                                    @if ($hash != null)
                                    <input type="text" class="py-1 w-20" wire:model.live="cost.{{ $index }}">
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    @if ($hash == null)
                                    <button type="button" class="px-2 py-1 text-xs text-stone-100 bg-red-700 rounded-lg" wire:click="removeRow({{ $index }})">Remove</button>
                                    @endif
                                </td>
                            </x-static-tr>
                            @endforeach
                        ...
                        <x-static-tr>
                            ...
                            <td colspan="3" class="py-1">
                                <x-button type="button" class="float-end text-xs px-1 py-1" wire:click="addRow()">Add
                                    Row</x-button>
                            </td>
                            @endif
                        </x-static-tr>
                ...
                <div class="w-full">
                    <x-select-input wireModel="assignTo" label="Assign to" :data="$userData" wire:model="assignTo" />
                </div>
                @endif
                @else
                <div class="w-full">
                    <x-input type="file" label="Select Attachments" wireModel="attachments" wire:model="attachments" multiple />
                </div>
                @endif
